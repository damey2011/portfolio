...
function Window(props) {
    ...
    return (
            <div className={`md:w-3/4 md:h-3/4 w-screen relative md:min-h-1/2 h-3/4vh bg-back-secondary
                ...
                <div className="bg-back-primary h-5 w-100 flex justify-between absolute w-full top-0 left-0"
                     onDoubleClick={e => toggleFullScreen(e, isFullScreen === '1' ? '0' : '1')}>
                    <div className="flex items-center window-top handle">
                        <Link to="/" className="ml-3 hover:bg-red-400 w-3 h-3 bg-red-500 rounded-full fas"/>
                        <a href='/' onClick={(e) => toggleFullScreen(e, '0')} className="ml-2 hover:bg-yellow-400 w-3 h-3 bg-yellow-500 rounded-full"> </a>
                        <a href="/" onClick={(e) => toggleFullScreen(e, '1')} className="ml-2 hover:bg-green-400 w-3 h-3 bg-green-500 rounded-full fas"> </a>
                    </div>
                    <div className="absolute transform -translate-x-1/2 left-1/2 flex items-center text-xs">
